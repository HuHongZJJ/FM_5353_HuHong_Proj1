...
namespace MyApp
{
    public class BarrierOption: Simulator
    {
        ...
        public double BS_delta(Barrier O, double j, double S)
        {
                double d1 = 0.0;
                double delta_bs = 0.0;
        ...
        public (double VP, double std) OptionPandStd(Barrier O)
        {
                ...
                if (O.S.IsAntithetic == false)
                {
                    ...
                    {   
                        ...
                        {   
                            ...
                            if (O.KnockType == KnockType.UpAndIn)
                            {
                                CT = Math.Max(0, St - O.Strike)*(SMaxNorm[i] > O.BarrierLevel ? 1:0) + beta1*CV;
                            }
                            else if(O.KnockType == KnockType.UpAndOut)
                            {
                                CT = Math.Max(0, St - O.Strike)*(SMaxNorm[i] < O.BarrierLevel ? 1:0) + beta1*CV;

                            }
                            else if(O.KnockType == KnockType.DownAndIn)
                            {
                                CT = Math.Max(0, St - O.Strike)*(SMinNorm[i] < O.BarrierLevel ? 1:0) + beta1*CV;
                            }
                            else if(O.KnockType == KnockType.DownAndOut)
                            {
                                CT = Math.Max(0, St - O.Strike)*(SMinNorm[i] > O.BarrierLevel ? 1:0) + beta1*CV;
                            }
                            sum_VT = sum_VT + CT;
                        ...
                        else if (O.IsCall == false)
                        {
                            ...
                            if (O.KnockType == KnockType.UpAndIn)
                            {
                                PT = Math.Max(0, O.Strike - St)*(SMaxNorm[i] > O.BarrierLevel ? 1:0) + beta1*PV;
                            ...
                            {
                                PT = Math.Max(0, O.Strike - St)*(SMinNorm[i] < O.BarrierLevel ? 1:0) + beta1*PV;
                ...
                if (O.S.IsAntithetic == true)
                {
                    ...
                    {   
                        ...
                        {   
                            ...
                            if (O.KnockType == KnockType.UpAndIn)
                            {
                                CT =0.5*( Math.Max(0, St - O.Strike)*(SMaxNorm[i] > O.BarrierLevel ? 1:0) + beta1*CV + Math.Max(0, St_2 - O.Strike)*(SMaxAnti[i] > O.BarrierLevel ? 1:0) + beta1*CV_2);
                            }
                            else if(O.KnockType == KnockType.UpAndOut)
                            {
                                CT =0.5*( Math.Max(0, St - O.Strike)*(SMaxNorm[i] < O.BarrierLevel ? 1:0) + beta1*CV + Math.Max(0, St_2 - O.Strike)*(SMaxAnti[i] < O.BarrierLevel ? 1:0) + beta1*CV_2);
                            ...
                            {
                                CT =0.5*( Math.Max(0, St - O.Strike)*(SMinNorm[i] < O.BarrierLevel ? 1:0) + beta1*CV + Math.Max(0, St_2 - O.Strike)*(SMinAnti[i] < O.BarrierLevel ? 1:0) + beta1*CV_2);
                        ...
                        else if (O.IsCall == false)
                        {
                            ...
                            if (O.KnockType == KnockType.UpAndIn)
                            {
                                PT =0.5*( Math.Max(0, O.Strike - St)*(SMaxNorm[i] > O.BarrierLevel ? 1:0) + beta1*PV + Math.Max(0, O.Strike- St_2)*(SMaxAnti[i] > O.BarrierLevel ? 1:0) + beta1*PV_2);
                            }
                            else if(O.KnockType == KnockType.UpAndOut)
                            {
                                PT =0.5*( Math.Max(0, O.Strike - St)*(SMaxNorm[i] < O.BarrierLevel ? 1:0) + beta1*PV + Math.Max(0, O.Strike - St_2)*(SMaxAnti[i] < O.BarrierLevel ? 1:0) + beta1*PV_2);
                            ...
                            {
                                PT =0.5*( Math.Max(0, O.Strike - St)*(SMinNorm[i] < O.BarrierLevel ? 1:0) + beta1*PV + Math.Max(0, O.Strike - St_2)*(SMinAnti[i] < O.BarrierLevel ? 1:0) + beta1*PV_2);
            ...
            else if (O.S.IsDeltaCV == false)
            {
                for (int i = 0; i < O.S.Simulations; i++)
                {
                    ...
                    {   
                        if (O.KnockType == KnockType.UpAndIn)
                        {
                            eachOption.Add(Math.Max(SpathNorm.SimulatedPaths[i, O.S.Steps - 1]-O.Strike, 0)*(SMaxNorm[i] > O.BarrierLevel ? 1:0));
                        }
                        else if(O.KnockType == KnockType.UpAndOut)
                        {
                            eachOption.Add(Math.Max(SpathNorm.SimulatedPaths[i, O.S.Steps - 1]-O.Strike, 0)*(SMaxNorm[i] < O.BarrierLevel ? 1:0));
                        ...
                        {
                            eachOption.Add(Math.Max(SpathNorm.SimulatedPaths[i, O.S.Steps - 1]-O.Strike, 0)*(SMinNorm[i] < O.BarrierLevel ? 1:0));
                    ...
                    else if (O.IsCall == false)
                    {
                        if (O.KnockType == KnockType.UpAndIn)
                        {
                            eachOption.Add(Math.Max(O.Strike - SpathNorm.SimulatedPaths[i, O.S.Steps - 1], 0)*(SMaxNorm[i] > O.BarrierLevel ? 1:0));
                        ...
                        {
                            eachOption.Add(Math.Max(O.Strike - SpathNorm.SimulatedPaths[i, O.S.Steps - 1], 0)*(SMinNorm[i] < O.BarrierLevel ? 1:0));
                ...
                if (O.S.IsAntithetic == true)
                {
                    for (int i = 0; i < O.S.Simulations; i++)
                    {
                        if (O.IsCall == true)
                        {
                            reverseOption.Add(Math.Max(SpathAnti.SimulatedPaths[i, O.S.Steps - 1]-O.Strike, 0));
                            if (O.KnockType == KnockType.UpAndIn)
                            {
                                reverseOption.Add(Math.Max(SpathAnti.SimulatedPaths[i, O.S.Steps - 1]-O.Strike, 0)*(SMaxAnti[i] > O.BarrierLevel ? 1:0));
                            ...
                            {
                                reverseOption.Add(Math.Max(SpathAnti.SimulatedPaths[i, O.S.Steps - 1]-O.Strike, 0)*(SMinAnti[i] < O.BarrierLevel ? 1:0));
                        ...
                        {   
                            reverseOption.Add(Math.Max(O.Strike - SpathAnti.SimulatedPaths[i, O.S.Steps - 1], 0));
                            if (O.KnockType == KnockType.UpAndIn)
                            {
                                reverseOption.Add(Math.Max(O.Strike - SpathAnti.SimulatedPaths[i, O.S.Steps - 1], 0)*(SMaxAnti[i] > O.BarrierLevel ? 1:0));
                            ...
                            {
                                reverseOption.Add(Math.Max(O.Strike - SpathAnti.SimulatedPaths[i, O.S.Steps - 1], 0)*(SMinAnti[i] < O.BarrierLevel ? 1:0));
        ...
        public double StandardError(Barrier O, bool IsAnti)
        {
            double SE = new double();
            Barrier O1 = new Barrier();
            ...
            O1.BarrierLevel = O.BarrierLevel;
            O1.KnockType = O.KnockType;
            O1.IsCall = O.IsCall;
            O1.S =S1;   
        ...
        public (double Delta, double Gamma) DeltaandGamma(Barrier O)
        {
            double deltaS = 0.1;
            double Delta = new double();
            double Gamma = new double();
            Barrier O1 = new Barrier();
            Barrier O2 = new Barrier();
            SimulationParameters S1 = new SimulationParameters();
            ...
            O2.BarrierLevel = O.BarrierLevel;
            O2.KnockType = O.KnockType;
            O2.IsCall = O.IsCall;
            O2.S = S2;
        ...
        public double VegaValue(Barrier O)
        {
        ...
        public double ThetaValue(Barrier O)
        {
            double deltaT = 0.01;
        ...
        public double RhoValue(Barrier O)
        {
