...
{
    public class LookbackOption: Simulator
    {
        ...
        public double BS_delta(Lookback O, double j, double S)
        {
                double d1 = 0.0;
                double delta_bs = 0.0;
        ...
        public (double VP, double std) OptionPandStd(Lookback O)
        {
                ...
                if (O.S.IsAntithetic == false)
                {
                    ...
                    {   
                        ...
                        {   
                            ...
                            CT = Math.Max(0, SMaxNorm[i] - O.Strike) + beta1*CV;
                            sum_VT = sum_VT + CT;
                        ...
                        else if (O.IsCall == false)
                        {
                            ...
                            PT = Math.Max(0,  O.Strike - SMinNorm[i]) + beta1*PV;
                            sum_VT = sum_VT + PT;
                ...
                if (O.S.IsAntithetic == true)
                {
                    ...
                    {   
                        ...
                        {   
                            ...
                            CT = 0.5* (Math.Max(0, SMaxNorm[i] - O.Strike) + beta1*CV + Math.Max(0, SMaxAnti[i] - O.Strike) + beta1*CV_2);
                            sum_VT = sum_VT + CT;
                            sum_VT2 = sum_VT2 + CT*CT;
                        }
                        else if (O.IsCall == false)
                        {
                            ...
                            PT = 0.5* (Math.Max(0, O.Strike - SMinNorm[i]) + beta1*PV + Math.Max(0,  O.Strike - SMinAnti[i]) + beta1*PV_2);
                            sum_VT = sum_VT + PT;
                            sum_VT2 = sum_VT2 + PT * PT;
                        }
            ...
            else if (O.S.IsDeltaCV == false)
            {
                for (int i = 0; i < O.S.Simulations; i++)
                {
                    if (O.IsCall == true)
                    {   
                        eachOption.Add(Math.Max(SMaxNorm[i]- O.Strike, 0));
                    }
                
                    else if (O.IsCall == false)
                    {
                        eachOption.Add(Math.Max(O.Strike - SMinNorm[i], 0));
                    }
                ...
                if (O.S.IsAntithetic == true)
                {
                    for (int i = 0; i < O.S.Simulations; i++)
                    {
                        if (O.IsCall == true)
                        {
                            reverseOption.Add(Math.Max(SMaxAnti[i]-O.Strike, 0));
                        }
                
                        else if (O.IsCall == false)
                        {   
                            reverseOption.Add(Math.Max(O.Strike - SMinAnti[i], 0));
                        } 
                    }
        ...
        public double StandardError(Lookback O, bool IsAnti)
        {
            double SE = new double();
            Lookback O1 = new Lookback();
        ...
        public (double Delta, double Gamma) DeltaandGamma(Lookback O)
        {
            double deltaS = 0.1;
            double Delta = new double();
            double Gamma = new double();
            Lookback O1 = new Lookback();
            Lookback O2 = new Lookback();
            SimulationParameters S1 = new SimulationParameters();
        ...
        public double VegaValue(Lookback O)
        {
        ...
        public double ThetaValue(Lookback O)
        {
            double deltaT = 0.01;
        ...
        public double RhoValue(Lookback O)
        {
